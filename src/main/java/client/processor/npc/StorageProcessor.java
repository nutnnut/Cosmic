...
public class StorageProcessor {
    ...
    public static void storageAction(InPacket p, Client c) {
        if (c.getPlayer().getUsingOreStorage()) {
            oreStorageAction(p, c);
            return;
        }
    ...
    private static void defaultStorageAction(InPacket p, Client c) {
    ...
    private static void oreStorageAction(InPacket p, Client c) {
        ItemInformationProvider ii = ItemInformationProvider.getInstance();
        Character chr = c.getPlayer();
        OreStorage storage = chr.getOreStorage();
            ...
            try {
                switch (mode) {
                    ...
                    case 5: { // Store
                        ...
                        if (!ItemConstants.isOreBagAllowed(itemId)) {
                            chr.dropMessage(1, "OreBag cannot hold that item!");
                            chr.sendPacket(PacketCreator.enableActions());
                            c.sendPacket(PacketCreator.enableActions());
                            storage.sendStored(c, ItemConstants.getInventoryType(itemId));
                            return;
                    ...
                    case 6: // Arrange items
                        ...
                        c.sendPacket(PacketCreator.enableActions());
                        break;
                    case 7: { // Mesos (blocked for ore storage)

...
public class OreStorage {
    ...
    private OreStorage(int id, byte slots, int meso) {
        this.id = id;
        this.slots = slots;
        this.meso = meso;
    }
    ...
    public static OreStorage loadOrCreateFromDB(int id, int world) {
        ...
        try (Connection con = DatabaseConnection.getConnection();
            ...
            try (ResultSet rs = ps.executeQuery()) {
                if (rs.next()) {
                    ret = new OreStorage(rs.getInt("storageid"), (byte) rs.getInt("slots"), rs.getInt("meso"));
            ...
            log.error("SQL error occurred when trying to load ore storage for accId {}, world {}", id, GameConstants.WORLD_NAMES[world], ex);

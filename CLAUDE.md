...
7. WZ DATA SAFETY (operational): worktrees are fine to USE — the danger is DELETING them. `wz/` is gitignored, large, and absent from fresh worktrees, so an agent may junction/symlink `wz` into its worktree to run WZ-backed tests. NEVER `git worktree remove`/`rm -rf` a worktree that contains such a junction — on Windows the recursive delete follows the reparse point and WIPES the real `wz/` target (this happened once). Leave worktrees in place for the user to clean up manually; if you must remove one, delete any `wz` junction with `rmdir` (not `rm -rf`) FIRST. WZ-backed tests are nav/graph-adjacent and usually skipped (rule #4), so the junction is rarely worth creating.
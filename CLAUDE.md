...
10. Verify NPC, item, map, quest, and skill ids against the handbook, WZ data, or another authoritative source. Never guess ids.
11. Bot chat is encoded as US-ASCII by the v83 client. Keep player-visible bot text ASCII-only; repository Markdown remains UTF-8.
12. Client-side calculations needed only to simulate a headless bot belong under `server.bots.*`. Do not change tested shared player paths such as `StatEffect` merely to compensate for a bot-side formula or packet bug.
13. BOT BACKLOG: `docs/bot/ROADMAP.md` is the only repo-level list of unfinished bot work. Add an item only when the owner explicitly wants it retained. Source markers must use `TODO(bot-<roadmap-id>)` and correspond to a roadmap row. KB limitations are facts, not tasks. Delete completed or abandoned rows; Git history replaces completion checklists and handoffs.

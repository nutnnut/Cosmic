...
7. WZ DATA SAFETY (operational): `wz/` is gitignored, large, and absent from fresh git worktrees. Do NOT create a junction/symlink named `wz` inside a worktree to make WZ-backed tests pass, and do NOT `git worktree remove --force` a worktree that contains such a junction — on Windows the recursive delete follows the reparse point and WIPES the real `wz/` target. Prefer running agents directly in the main tree (no `isolation: worktree`) for this repo; if a worktree is truly needed, remove any `wz` junction with `rmdir` (not `rm -rf`) BEFORE removing the worktree. WZ-backed tests are nav/graph-adjacent anyway and are usually skipped (rule #4).
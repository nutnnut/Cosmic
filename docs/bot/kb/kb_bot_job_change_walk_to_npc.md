...
**Mushroom Shrine without a saved world-tour origin (live `fifty1958`, 2026-07-13).** A character can be persisted at `800000000` without a `savedlocations.WORLDTOUR` row (for example, after being placed there or after state loss). The world graph previously treated the shrine as a dead end, so a job-advance errand held forever with `route-reachable=false` and `JOB_CHANGE_FALLBACK_ANYWHERE=false`. The real Spinel script (`scripts/npc/9000020.js`) defaults a missing return map to Lith Harbor `104000000`; bot routing now uses the same fallback. Route-null job errands are also retained as a server-side `stuck` reason so the web roster's possibly-stuck counter includes them. Regression coverage lives in `BotAutopilotManagerTest`.

The same fallback applies when the persisted `WORLDTOUR` points back into Zipangu itself (live `MomentLol`, saved `800000000`, 2026-07-20). Spinel can only board for Mushroom Shrine from outside continent 8, so such a value is invalid/corrupt; accepting it makes the Shrine exit a self-loop and every mainland job route null. `BotAutopilotManager.worldTourReturnOrFallback` is the SSOT used by both route planning and `BotTravelManager.taxiRide`: missing or continent-8 origins return to Lith Harbor.

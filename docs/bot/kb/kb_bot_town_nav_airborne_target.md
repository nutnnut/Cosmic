...
**Verification:** compile clean; BotTravelManagerTest 25/25 (incl. 4 new: portal filter, resetForModeChange-clears / clear-keeps cooldown, progress-extends-deadline); broad bot sweep green EXCEPT BotPhysicsEngineTest.shouldRefuseDownJumpWhenTheNextFloorIsTooFarBelow — PRE-EXISTING failure (fails on clean baseline too; stale vs the removed 300px down-jump cap, GRAPH_VERSION 57, see [[kb_bot_downjump_eligibility]]). Live in-game repro still pending (user).

**Concurrent-session hazard (recurring):** during this work another session left uncommitted edits in BotManager/BotBuildManager/BotEquipManager/BotScrollManager/BotStarterKitManager (+tests, docs/bot/ownerless-bots-design.md). `git stash -u` swept them in; committed ONLY my 5 files by explicit path. When stashing, beware other sessions' in-tree work.

...
FIX (2026-06-30): Zipangu interior was a one-way routing trap. The Spinel return out of Mushroom Shrine 800000000 is synthesized in `BotWorldGraph.expand()` only when `options.worldTourReturn() != -1`, and the SSOT producer `BotAutopilotManager.worldTourReturn(bot)` keyed on the bot standing on the shrine TILE (`mapId == MUSHROOM_SHRINE`). A bot that rode Spinel in (saving `WORLDTOUR`) then walked deeper into Zipangu (e.g. lv30 London on Crow field 800020101, save=Orbis 200000000) got `-1` from any interior map → route planning never added the Spinel exit → `routeForBot` null → stuck forever (job-errand, fallback OFF). Fix: gate on the whole Zipangu CONTINENT instead of the tile — `mapId / 100000000 == MUSHROOM_SHRINE / 100000000` (continent 8). Continent 8 is an island whose ONLY entry is the Spinel ride (which saves WORLDTOUR) and whose death/relog keeps the bot in-continent, so any bot inside has a valid origin; a stale mainland save still gets `-1`, so it can't reopen the shrine as a through-shortcut to Lith. (A returnMap-based gate was tried first but misses **Showa** town 801000000, which death-returns to itself, not the shrine — Showa is a dead end too: a bot there must walk Showa → Mushroom Shrine → Spinel out. Portal BFS confirmed Showa and the 800xxxxx fields are walk-connected to the shrine; the 800040xxx scripted dungeon block and 889xxxxx event maps are not, so they never benefit — that's fine, they're separate scripted-entrance issues.) Also fixed the `BotStarterKitManager` stuck-warn `RouteOptions` (was 5-arg, defaulted worldTourReturn=-1) to pass `worldTourReturn(bot)` so the logged `route-reachable=` matches real reachability.

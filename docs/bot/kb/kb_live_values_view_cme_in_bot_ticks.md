...
**How to apply:** never iterate a live `.values()` / `.list()` / unmodifiable-view return off the owning thread. Snapshot under the lock (as `list()` now does), or use existing snapshot accessors (`listById`, `getAllMonsters`, `getAllPlayers`, `getMapObjectsInRange` all copy under lock). Also: server logs truncate to 2 JDK frames — get the full stacktrace first; the real caller frame (`BotPotionManager:83`) only appeared once the full trace was available.
